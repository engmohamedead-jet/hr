...
const service = {
  ...
  accountTransactions: () => FIND_MANY_RESULT,
  accountTransaction: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("AccountTransaction", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: AccountTransactionService,
          useValue: service,
        },
      ],
  ...
  test("POST /accountTransactions", async () => {
  ...
  test("GET /accountTransactions", async () => {
    await request(app.getHttpServer())
  ...
  test("GET /accountTransactions/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/accountTransactions"}/${nonExistingId}`)
  ...
  test("GET /accountTransactions/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/accountTransactions"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /accountTransactions existing resource", async () => {
