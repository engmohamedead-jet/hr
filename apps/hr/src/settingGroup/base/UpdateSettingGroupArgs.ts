/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SettingGroupWhereUniqueInput } from "./SettingGroupWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { SettingGroupUpdateInput } from "./SettingGroupUpdateInput";

@ArgsType()
class UpdateSettingGroupArgs {
  @ApiProperty({
    required: true,
    type: () => SettingGroupWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => SettingGroupWhereUniqueInput)
  @Field(() => SettingGroupWhereUniqueInput, { nullable: false })
  where!: SettingGroupWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => SettingGroupUpdateInput,
  })
  @ValidateNested()
  @Type(() => SettingGroupUpdateInput)
  @Field(() => SettingGroupUpdateInput, { nullable: false })
  data!: SettingGroupUpdateInput;
}

export { UpdateSettingGroupArgs as UpdateSettingGroupArgs };
