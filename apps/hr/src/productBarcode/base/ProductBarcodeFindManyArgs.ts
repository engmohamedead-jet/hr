/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProductBarcodeWhereInput } from "./ProductBarcodeWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ProductBarcodeOrderByInput } from "./ProductBarcodeOrderByInput";

@ArgsType()
class ProductBarcodeFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ProductBarcodeWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ProductBarcodeWhereInput, { nullable: true })
  @Type(() => ProductBarcodeWhereInput)
  where?: ProductBarcodeWhereInput;

  @ApiProperty({
    required: false,
    type: [ProductBarcodeOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ProductBarcodeOrderByInput], { nullable: true })
  @Type(() => ProductBarcodeOrderByInput)
  orderBy?: Array<ProductBarcodeOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ProductBarcodeFindManyArgs as ProductBarcodeFindManyArgs };
