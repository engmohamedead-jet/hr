/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { WorkCenterRoutingWhereInput } from "./WorkCenterRoutingWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class WorkCenterRoutingListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => WorkCenterRoutingWhereInput,
  })
  @ValidateNested()
  @Type(() => WorkCenterRoutingWhereInput)
  @IsOptional()
  @Field(() => WorkCenterRoutingWhereInput, {
    nullable: true,
  })
  every?: WorkCenterRoutingWhereInput;

  @ApiProperty({
    required: false,
    type: () => WorkCenterRoutingWhereInput,
  })
  @ValidateNested()
  @Type(() => WorkCenterRoutingWhereInput)
  @IsOptional()
  @Field(() => WorkCenterRoutingWhereInput, {
    nullable: true,
  })
  some?: WorkCenterRoutingWhereInput;

  @ApiProperty({
    required: false,
    type: () => WorkCenterRoutingWhereInput,
  })
  @ValidateNested()
  @Type(() => WorkCenterRoutingWhereInput)
  @IsOptional()
  @Field(() => WorkCenterRoutingWhereInput, {
    nullable: true,
  })
  none?: WorkCenterRoutingWhereInput;
}
export { WorkCenterRoutingListRelationFilter as WorkCenterRoutingListRelationFilter };
