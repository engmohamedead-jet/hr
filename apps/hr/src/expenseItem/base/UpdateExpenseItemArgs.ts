/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ExpenseItemWhereUniqueInput } from "./ExpenseItemWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ExpenseItemUpdateInput } from "./ExpenseItemUpdateInput";

@ArgsType()
class UpdateExpenseItemArgs {
  @ApiProperty({
    required: true,
    type: () => ExpenseItemWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ExpenseItemWhereUniqueInput)
  @Field(() => ExpenseItemWhereUniqueInput, { nullable: false })
  where!: ExpenseItemWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ExpenseItemUpdateInput,
  })
  @ValidateNested()
  @Type(() => ExpenseItemUpdateInput)
  @Field(() => ExpenseItemUpdateInput, { nullable: false })
  data!: ExpenseItemUpdateInput;
}

export { UpdateExpenseItemArgs as UpdateExpenseItemArgs };
