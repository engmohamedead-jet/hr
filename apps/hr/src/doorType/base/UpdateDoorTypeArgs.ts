/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DoorTypeWhereUniqueInput } from "./DoorTypeWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { DoorTypeUpdateInput } from "./DoorTypeUpdateInput";

@ArgsType()
class UpdateDoorTypeArgs {
  @ApiProperty({
    required: true,
    type: () => DoorTypeWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => DoorTypeWhereUniqueInput)
  @Field(() => DoorTypeWhereUniqueInput, { nullable: false })
  where!: DoorTypeWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => DoorTypeUpdateInput,
  })
  @ValidateNested()
  @Type(() => DoorTypeUpdateInput)
  @Field(() => DoorTypeUpdateInput, { nullable: false })
  data!: DoorTypeUpdateInput;
}

export { UpdateDoorTypeArgs as UpdateDoorTypeArgs };
