/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SaleOrderDetailWhereInput } from "./SaleOrderDetailWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SaleOrderDetailOrderByInput } from "./SaleOrderDetailOrderByInput";

@ArgsType()
class SaleOrderDetailFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SaleOrderDetailWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SaleOrderDetailWhereInput, { nullable: true })
  @Type(() => SaleOrderDetailWhereInput)
  where?: SaleOrderDetailWhereInput;

  @ApiProperty({
    required: false,
    type: [SaleOrderDetailOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SaleOrderDetailOrderByInput], { nullable: true })
  @Type(() => SaleOrderDetailOrderByInput)
  orderBy?: Array<SaleOrderDetailOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SaleOrderDetailFindManyArgs as SaleOrderDetailFindManyArgs };
