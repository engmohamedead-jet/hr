/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { InstallmentSaleFeeWhereInput } from "./InstallmentSaleFeeWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { InstallmentSaleFeeOrderByInput } from "./InstallmentSaleFeeOrderByInput";

@ArgsType()
class InstallmentSaleFeeFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => InstallmentSaleFeeWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => InstallmentSaleFeeWhereInput, { nullable: true })
  @Type(() => InstallmentSaleFeeWhereInput)
  where?: InstallmentSaleFeeWhereInput;

  @ApiProperty({
    required: false,
    type: [InstallmentSaleFeeOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [InstallmentSaleFeeOrderByInput], { nullable: true })
  @Type(() => InstallmentSaleFeeOrderByInput)
  orderBy?: Array<InstallmentSaleFeeOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { InstallmentSaleFeeFindManyArgs as InstallmentSaleFeeFindManyArgs };
