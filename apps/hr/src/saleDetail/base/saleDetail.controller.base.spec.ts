...
const service = {
  ...
  saleDetails: () => FIND_MANY_RESULT,
  saleDetail: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("SaleDetail", () => {
  ...
  test("GET /saleDetails/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/saleDetails"}/${nonExistingId}`)
  ...
  test("GET /saleDetails/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/saleDetails"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /saleDetails existing resource", async () => {
