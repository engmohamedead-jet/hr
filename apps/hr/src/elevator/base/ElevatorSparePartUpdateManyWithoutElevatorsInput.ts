/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ElevatorSparePartWhereUniqueInput } from "../../elevatorSparePart/base/ElevatorSparePartWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class ElevatorSparePartUpdateManyWithoutElevatorsInput {
  @Field(() => [ElevatorSparePartWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ElevatorSparePartWhereUniqueInput],
  })
  connect?: Array<ElevatorSparePartWhereUniqueInput>;

  @Field(() => [ElevatorSparePartWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ElevatorSparePartWhereUniqueInput],
  })
  disconnect?: Array<ElevatorSparePartWhereUniqueInput>;

  @Field(() => [ElevatorSparePartWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ElevatorSparePartWhereUniqueInput],
  })
  set?: Array<ElevatorSparePartWhereUniqueInput>;
}

export { ElevatorSparePartUpdateManyWithoutElevatorsInput as ElevatorSparePartUpdateManyWithoutElevatorsInput };
