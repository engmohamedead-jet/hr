/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LeaveRequestWhereUniqueInput } from "./LeaveRequestWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { LeaveRequestUpdateInput } from "./LeaveRequestUpdateInput";

@ArgsType()
class UpdateLeaveRequestArgs {
  @ApiProperty({
    required: true,
    type: () => LeaveRequestWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => LeaveRequestWhereUniqueInput)
  @Field(() => LeaveRequestWhereUniqueInput, { nullable: false })
  where!: LeaveRequestWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => LeaveRequestUpdateInput,
  })
  @ValidateNested()
  @Type(() => LeaveRequestUpdateInput)
  @Field(() => LeaveRequestUpdateInput, { nullable: false })
  data!: LeaveRequestUpdateInput;
}

export { UpdateLeaveRequestArgs as UpdateLeaveRequestArgs };
