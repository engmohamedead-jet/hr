/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PaymentStatusWhereUniqueInput } from "./PaymentStatusWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { PaymentStatusUpdateInput } from "./PaymentStatusUpdateInput";

@ArgsType()
class UpdatePaymentStatusArgs {
  @ApiProperty({
    required: true,
    type: () => PaymentStatusWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => PaymentStatusWhereUniqueInput)
  @Field(() => PaymentStatusWhereUniqueInput, { nullable: false })
  where!: PaymentStatusWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => PaymentStatusUpdateInput,
  })
  @ValidateNested()
  @Type(() => PaymentStatusUpdateInput)
  @Field(() => PaymentStatusUpdateInput, { nullable: false })
  data!: PaymentStatusUpdateInput;
}

export { UpdatePaymentStatusArgs as UpdatePaymentStatusArgs };
