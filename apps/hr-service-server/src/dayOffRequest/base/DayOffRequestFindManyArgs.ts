/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DayOffRequestWhereInput } from "./DayOffRequestWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { DayOffRequestOrderByInput } from "./DayOffRequestOrderByInput";

@ArgsType()
class DayOffRequestFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DayOffRequestWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => DayOffRequestWhereInput, { nullable: true })
  @Type(() => DayOffRequestWhereInput)
  where?: DayOffRequestWhereInput;

  @ApiProperty({
    required: false,
    type: [DayOffRequestOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [DayOffRequestOrderByInput], { nullable: true })
  @Type(() => DayOffRequestOrderByInput)
  orderBy?: Array<DayOffRequestOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DayOffRequestFindManyArgs as DayOffRequestFindManyArgs };
