/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { EmployeeDepartmentWhereUniqueInput } from "../../employeeDepartment/base/EmployeeDepartmentWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class EmployeeDepartmentUpdateManyWithoutTenantsInput {
  @Field(() => [EmployeeDepartmentWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [EmployeeDepartmentWhereUniqueInput],
  })
  connect?: Array<EmployeeDepartmentWhereUniqueInput>;

  @Field(() => [EmployeeDepartmentWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [EmployeeDepartmentWhereUniqueInput],
  })
  disconnect?: Array<EmployeeDepartmentWhereUniqueInput>;

  @Field(() => [EmployeeDepartmentWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [EmployeeDepartmentWhereUniqueInput],
  })
  set?: Array<EmployeeDepartmentWhereUniqueInput>;
}

export { EmployeeDepartmentUpdateManyWithoutTenantsInput as EmployeeDepartmentUpdateManyWithoutTenantsInput };
